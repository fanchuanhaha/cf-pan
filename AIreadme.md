...
- 全新安装已增加“文件上传方式”和“文件下载方式”：可选择网站代理或存储直传/直链，并保存 `uploadfile_type`、`downfile_type`、`downfile_protocol`、`downfile_domain` 到 D1。
- 从备份恢复第二步也会读取 SQL 中对应的 `uploadfile_type`、`downfile_type`、`downfile_protocol`、`downfile_domain` 值并显示，可修改后随 `config-apply` 写入 D1；未出现在 SQL 中时使用网站代理上传/下载默认值。
