...
/**
 * 根据 pre_config 中已有的字段，智能识别原系统实际使用的存储后端
 * 用于：原 PHP 项目 storage='local' 时（不支持），通过检测 *_ak 等字段推断真实使用的存储
 * 返回：{ type, fields, source: 'pre_config'|'inferred', confidence }
 *   - type: 'r2' | 's3' | 'github' | 'webdav' | 'upyun' | 'qiniu' | null
 *   - fields: 提取出的该存储所需字段（已去除前缀，可直接写入 D1）
 *   - source: 'pre_config' 表示 storage 字段已正确；'inferred' 表示根据其它字段推断
 *   - confidence: 'high' (有 AK/SK) | 'medium' (有 bucket 等但无密钥) | 'low'
 */
export function detectStorageFromPreConfig(preConfig: PreConfigMap): {
  type: 'r2' | 's3' | 'github' | 'webdav' | 'upyun' | 'qiniu' | null;
  fields: PreConfigMap;
  source: 'pre_config' | 'inferred';
  confidence: 'high' | 'medium' | 'low';
} {
  const validTypes = ['r2', 's3', 'github', 'webdav', 'upyun', 'qiniu'];
  const declared = preConfig['storage'];
  // 1) 优先用 pre_config 显式声明的 storage（如果不是 local 或非法值）
  if (declared && validTypes.includes(declared)) {
    return {
      type: declared as any,
      fields: extractStorageFields(preConfig, declared),
      source: 'pre_config',
      confidence: 'high',
    };
  }
  // 2) 推断：根据 *_ak / *_token / *_endpoint 等关键字段判断
  const candidates: { type: string; score: number; requires: string[] }[] = [
    { type: 'qiniu', score: 0, requires: ['qiniu_ak', 'qiniu_sk', 'qiniu_bucket'] },
    { type: 'upyun', score: 0, requires: ['upyun_bucket', 'upyun_operator', 'upyun_password'] },
    { type: 's3', score: 0, requires: ['s3_endpoint', 's3_bucket', 's3_ak', 's3_sk'] },
    { type: 'webdav', score: 0, requires: ['webdav_endpoint', 'webdav_user', 'webdav_pass'] },
    { type: 'github', score: 0, requires: ['gh_owner', 'gh_repo', 'gh_token'] },
  ];
  for (const c of candidates) {
    let hit = 0;
    for (const k of c.requires) {
      if (preConfig[k] && preConfig[k].length > 0) hit++;
    }
    c.score = hit / c.requires.length;
  }
  candidates.sort((a, b) => b.score - a.score);
  const best = candidates[0];
  if (best && best.score >= 0.5) {
    const confidence: 'high' | 'medium' | 'low' =
      best.score >= 1 ? 'high' : best.score >= 0.7 ? 'medium' : 'low';
    return {
      type: best.type as any,
      fields: extractStorageFields(preConfig, best.type),
      source: 'inferred',
      confidence,
    };
  }
  return { type: null, fields: {}, source: 'inferred', confidence: 'low' };
}

function extractStorageFields(preConfig: PreConfigMap, type: string): PreConfigMap {
  ...
  const keys = prefixMap[type] || [];
  for (const k of keys) {
    if (preConfig[k] != null && preConfig[k] !== '') out[k] = preConfig[k];
  }
  return out;
}
